<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto ">
			...
			<div class="md:col-span-12 lg:col-span-8 col-span-12">
				...
				<div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
					...
					<ul class="grid w-full gap-6 md:grid-cols-1">
						<li>
							<input wire:model='payment_method' class="hidden peer" id="midtrans" type="radio" value="midtrans" required>
							<label class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700" for="midtrans">
								<div class="block">
									...
									<div class="w-full text-sm">
										Kartu Kredit, Transfer Bank, E-Wallet, dan metode lainnya
									</div>
								</div>
								<svg aria-hidden="true" class="w-5 h-5 ms-3 rtl:rotate-180" fill="none" viewbox="0 0 14 10" xmlns="http://www.w3.org/2000/svg">
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			window.addEventListener('showPaymentDialog', function (event) {
				let snapToken = event.detail.snapToken;

				window.snap.pay(snapToken, {
					onSuccess: function (result) {
						Livewire.emit('handlePaymentSuccess', result);
					},
					onPending: function (result) {
						Livewire.emit('handlePaymentPending', result);
					},
					onError: function (result) {
						Livewire.emit('handlePaymentError', result);
					},
					onClose: function () {
